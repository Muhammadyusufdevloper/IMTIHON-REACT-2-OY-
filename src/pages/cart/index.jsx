...
const Cart = () => {
    ...
    let cartSlice = useSelector(state => state.cart.value)
    let dispatch = useDispatch()

    useEffect(() => {
        let total = cartSlice?.reduce((sum, el) => sum + (el.price * el.amount), 0)
        setTotalPrice(total)
    }, [cartSlice])
    useEffect(() => {
        scroll(0, 0)
    ...
    const handleVoucher = e => {
        e.preventDefault()
        setVoucher(inpValue === "laylo" ? totalPrice * 0.24 : toast.error("voucher not"))
    }
